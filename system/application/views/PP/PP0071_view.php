...
${$col}[1]		=	array("name"=>"machine_code","type"=>"string","editor"=>"machineCodeCombo,width:150","df"=>"" );
${$col}[2]		=	array("name"=>"machine_side","type"=>"string","editor"=>"{xtype:'combo', allowBlank:false, typeAhead: true,triggerAction: 'all',transform: 'machine_side',lazyRender: true, listClass: 'x-combo-list-small'},width:150","df"=>"" );

${$col}[3]		=	array("name"=>"finish_A_qty","type"=>"number","editor"=>"numFieldBlank,width:100","df"=>"" );
${$col}[4]		=	array("name"=>"finish_B_qty","type"=>"number","editor"=>"numFieldBlank,width:100","df"=>"" );
${$col}[5]		=	array("name"=>"lost_qty","type"=>"number","editor"=>"numFieldBlank,width:100","df"=>"" );
...
{
	cutEnableButton(false);
	var itemTxt = Ext.getCmp('ext-comp-1042');
...
// Main form 
	var leftComp  = [jobNoCombo,itemCode,notes];
...
// Final panel	
    ...
    function updateEvent(){
        ...
        if (fp.form.isValid()) {
			...
			if( cutDataStore.getCount() <= 0 || addClick == true || isEdit == true)
			{
            ...
            obj["data"] = new Object();
     	    ...
     	    for(var i = 0 ; i < cutDataStore.getCount() ; i++)
    	    {

        	    obj["data"][i] = new Object();
        	    obj["data"][i]["staff_code"] = cutDataStore.getAt(i).get("staff_code");
        	    obj["data"][i]["machine_code"] = cutDataStore.getAt(i).get("machine_code");
        	    obj["data"][i]["machine_side"] = cutDataStore.getAt(i).get("machine_side");
        	    obj["data"][i]["finish_A_qty"] = parseFloat(cutDataStore.getAt(i).get("finish_A_qty"));
        	    obj["data"][i]["finish_B_qty"] = parseFloat(cutDataStore.getAt(i).get("finish_B_qty"));
        	    obj["data"][i]["lost_qty"] = parseFloat(cutDataStore.getAt(i).get("lost_qty"));
