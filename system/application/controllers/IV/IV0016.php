 ...
 *------------------------------------------------------------------------------
 */
class IV0016 extends MY_Controller {
	...
	function getURReasonCombo()
	{
		...
		if($_POST != NULL) {
			...
			$ret['data'] = $this->common->getUrComboList()->result_array();		
		}
	...
	function getItemCode()
	{
		...
		if($_POST != NULL) {
			...
			$ret['success'] = true;
			$ret['message'] = "Get Item List";
			$data = $this->model->getItemCode($jobNo,$refDoc)->result_array();
			$ret['data'] = $data;
		}
		echo json_encode($ret);		
	}	
	
	function getLimitQty()
	{
		$ret = array();
		$ret['success'] = false;
		$ret['message'] = "Cannot get item list";
		if($_POST != NULL) {
			...
			$ret['success'] = true;
			$ret['message'] = "Get Item List";
			$data = $this->model->getLimitQty($jobNo,$refDoc)->result_array();
			$ret['data'] = $data;
		}
		echo json_encode($ret);		
	}	
	...
	function insertLedger()
	{
		...
		if($_POST != NULL) {
			...
			$recvReason = "ur_reason-7";//$this->input->post('ur_reason_val');
			...
			// Insert to Received ledger
				...
				$row->{'received_reason'} = $recvReason;
				$row->{'delete_flag'} = '0';
				$row->{'notes'} = 'notes';
