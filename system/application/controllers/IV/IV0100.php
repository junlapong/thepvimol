 ...
 *------------------------------------------------------------------------------
 */
class IV0100 extends MY_Controller {
	...
	function updateData()
	{
		...
		if($_POST != NULL) {
			...
			// UPDATE INVENTORY_MASTER (FIND ID)
				...
				// Receive in
				$rawInv = $this->model->getInvAt($item,$desLoca)->result_array();
				$invQty = $rawInv[0]['current_stock_qty'];
				$invId  = $rawInv[0]['id'];
			...
			// UPDATE INVENTORY_LOT_MASTER (ID)
				...
				// Receive IN
				$count = $this->model->getRecordCount($item,$desLoca,$lot,$seq);
				
				if ($count > 0)
				{	   // Update
					$rawInv = $this->model->getInvLotAt($item,$desLoca,$lot,$seq)->result_array();
					$invQty = $rawInv[0]['current_stock_qty'];
					$invId  = $rawInv[0]['id'];
					$inv = false;
					$inv->{'current_stock_qty'} = $invQty + $transQty;
