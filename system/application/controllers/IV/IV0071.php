 ...
 *------------------------------------------------------------------------------
 */
class IV0071 extends MY_Controller {
	...
	function getPoNoList()
	{
		...
		if($_POST != NULL) {
			...
			$data = $this->model->getPoNoList($query)->result_array();
			$ret['data'] = $data;
		}
	...
	function insertLedger()
	{
		...
		if($_POST != NULL) {
			...
			$refDoc   = $poNo; //$this->input->post('ref_doc_no');
			...
			// Update PURCHASE_REQUEST_LEDGER
				$this->model->updateReceiveDate($poNo,$recvDate,$item);
				
			// Update Production JOB (JOB_LEDGER) vvv
