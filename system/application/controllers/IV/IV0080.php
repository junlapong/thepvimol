 ...
 *------------------------------------------------------------------------------
 */
class IV0080 extends MY_Controller {
	...
	function getDeliveryNoList()
	{
		...
		if($_POST != NULL) {
			...
			$ret['success'] = true;
			$ret['message'] = "Get Item List";
			$data = $this->model->getDeliveryNoList($query)->result_array();
			$ret['data'] = $data;
		}
		echo json_encode($ret);			
	}
	...
	function getDeliveryDetail()
	{
		...
		if($_POST != NULL) {
			...
			$ret['success'] = true;
			$ret['message'] = "Get Item List";
			$data = $this->model->getDeliveryDetail($query)->result_array();
			$ret['data'] = $data;
		}
		echo json_encode($ret);			
	}

	function getReqOrderQty()
	{
		$ret = array();
		$ret['success'] = true;
		$ret['message'] = "...";
		...
		if($_POST != NULL) {
			$deli = $this->input->post('deli');
			$item = $this->input->post('item');
			$order = $this->input->post('order');
			$ret['success'] = true;
			$ret['message'] = "Get Item List";
			$data = $this->model->getReqOrderQty($deli,$item,$order)->result_array();
			$ret['data'] = $data;
		}
		echo json_encode($ret);		
	}
	
	function forecastJobNo()
	{
		$ret = array();
		$ret['success'] = true;
		$ret['message'] = "...";
		...
		$deli = $this->input->post('deli');
		//Get all Item ( by using delivery_no for search)
		$query = $this->model->getItemList($deli)->result_array();
		$i = 0;
		$k = 0;
		$data = array();
		foreach($query as $row)
		{
			//Search in INVENTORY_LOT_MASTER (FIFO) (by using item_code);
			$req_qty = $row["order_qty"];
			$fifoList = $this->model->getInvFIFO($row["item_code"])->result_array();
			$remainQty = $req_qty;
			foreach($fifoList as $fifo)
			{
				//print_r($fifoList);
				$data[$i]["id"] = $i;
				$data[$i]["item_code"] = $row["item_code"];
				$data[$i]["req_qty"] = $row["order_qty"];

				$data[$i]["location_code"] = $fifo["location_code"];
				$data[$i]["lot_no"] = $fifo["lot_no"];
				$data[$i]["seq_no"] = $fifo["seq_no"];

				$data[$i]["inv_qty"] = $fifo["current_stock_qty"];
			
				if( ($remainQty - $fifo["current_stock_qty"]) <= 0)
				{
					$data[$i]["issue_qty"] = $remainQty;
					$i = $i + 1;
					break;
				}else{
					$data[$i]["issue_qty"] = $fifo["current_stock_qty"];
					$remainQty = $remainQty - $fifo["current_stock_qty"];
				}
				$i = $i + 1;
			}
		}
	...
	function insertLedger()
	{
		...
		if($_POST != NULL) {
			...
			if($issuType == "P")
			{
					...
					$imaxUI = (int)$maxUI + 1;
					$maxUI = "PI" . str_pad((int) $imaxUI,8,"0",STR_PAD_LEFT);
					//echo $maxUR;
			}
			...
			$deliNo   = $this->input->post('delivery_no');
			...
			// Update Issue_flag in DELIVERY_LEDGER
				...
				$this->model->updateDeliveryLedger($deliNo,$item,$arr);				
