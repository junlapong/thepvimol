 ...
 *------------------------------------------------------------------------------
 */
class IV0022 extends MY_Controller {
	...
	function getItemList()
	{
		...
		if($_POST != NULL) {
			...
			// Use job no or screen no to find item_code
			$iData = $this->model->getItemCode($query)->result_array();
			
			$ret['success'] = true;
			$ret['message'] = "Get Item List";
			$data = $this->model->getItemList($iData[0]["item_code"])->result_array();
			$ret['data'] = $data;
		}
