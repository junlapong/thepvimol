 ...
 *------------------------------------------------------------------------------
 */
class MT0151 extends MY_Controller {
	...
	function viewDetail()
	{
		...
		if($_POST != NULL) {
			$saleCode = $this->input->post('sale_code');
			$ret['success'] = true;
			$ret['total'] = $this->model->getRowCountD($saleCode);
			$ret['message'] = "Loaded data";
			$data = $this->model->getDataDetail($saleCode)->result_array();
			$ret['data'] = $data; 
	...
	{
		$ret = array();
		$query = $this->input->post('sale_code');
		$ret['success'] = true;
