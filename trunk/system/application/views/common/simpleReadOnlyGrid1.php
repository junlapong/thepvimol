...
function deliDateRender(val){
	var today = new Date();
	var plandate = new Date(val);
	result = val;
	if(today > plandate)
	{
        result = '<span style="color:red;">' + val + '</span>';
	}
...
var <?=$gname?>ColumnModel = new Ext.ux.grid.LockingColumnModel({
	...
	columns: [
				...
				for($i=0; $i < count(${$gname}) ; $i++){
					...
					if( (strpos(${$gname}[$i]["name"],'p_lost_qty') !== false) ) echo ",renderer: per_lost1,align:'right'";
					if( (strpos(${$gname}[$i]["name"],'t_lost_qty') !== false) ) echo ",renderer: per_lost1,align:'right'";
					if( (strpos(${$gname}[$i]["name"],'plan_order_date') !== false) ) echo ",renderer: deliDateRender,align:'right'";
					//if( (strpos(${$gname}[$i]["name"],'order_date') !== false) ) echo ",renderer: deliDateRender,align:'right'";
