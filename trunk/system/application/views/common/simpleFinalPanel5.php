<?php 
	/*
	 *  Require Variable
	 *  Same as SimpleFinalPanel2 but have 3 Grid
	 *
	 *  $render = Render to
   ...
   // Final Panel
    var fp = new Ext.FormPanel({
        ...
        items: [
        	...
        	{
            	columnWidth: <?=$centerWidth?>,
	            layout: 'fit',
	            items: grid<?=$centerComp?>
        	},
        	{
    ...
    function actionAfterEvent(){
    	...
    	grid<?=$centerComp?>.setWidth((width-55)*<?=$centerComp?>);    		
    	grid<?=$rightComp?>.setWidth((width-55)*<?=$rightWidth?>);    		

    	grid<?=$leftComp?>.doLayout();
    	grid<?=$centerComp?>.doLayout();    		
